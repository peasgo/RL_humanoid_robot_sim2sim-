...
class MySceneCfg(InteractiveSceneCfg):
    ...
    height_scanner = RayCasterCfg(
        prim_path="{ENV_REGEX_NS}/Robot/FTB",
...
@configclass
class ActionsCfg:
    """只暴露 mimic 的 5 个主动关节（原来是 `joint_names=[".*"]`）。"""
...
class EventCfg:
    ...
    base_com = EventTerm(
        ...
        mode="startup",
        params={"asset_cfg": SceneEntityCfg("robot", body_names="base"), "com_range": {"x": (-0.05, 0.05), "y": (-0.05, 0.05), "z": (-0.01, 0.01)}},
    )
