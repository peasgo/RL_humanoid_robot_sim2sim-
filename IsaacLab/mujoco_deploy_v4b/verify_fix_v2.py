#!/usr/bin/env python3
"""验证修复：action_filter_alpha=0.0 后的行为测试"""
import numpy as np
import mujoco
import torch
import yaml
...
print(f"action_filter_alpha = {cfg['action_filter_alpha']}")
print(f"obs_filter_alpha = {cfg['obs_filter_alpha']}")
print(f"action_ramp_steps = {cfg['action_ramp_steps']}")
...
def run_sim(cmd_vec, num_steps=500):
    ...
    action_filter_alpha = cfg["action_filter_alpha"]
    obs_filter_alpha = cfg["obs_filter_alpha"]
    obs_filter_mode = cfg.get("obs_filter_mode", "vel_only")
    action_ramp_steps = cfg["action_ramp_steps"]
    action_clip = cfg.get("action_clip", None)
    cmd_scale = np.array(cfg.get("cmd_scale", [1,1,1]), dtype=np.float32)
...
print("\n" + "="*90)
print("验证修复：action_filter_alpha=0.0")
print("="*90)
...
print(f"\n{'命令':12s} | {'cmd':18s} | {'前进(m)':>8s} | {'横向(m)':>8s} | {'转向(°)':>8s} | {'高度(m)':>7s}")
print("-"*80)

for cname, cvec in cmds:
    fwd, lat, yaw, h = run_sim(cvec, num_steps=500)
    print(f"{cname:12s} | {str(cvec):18s} | {fwd:+8.3f} | {lat:+8.3f} | {yaw:+8.1f} | {h:7.3f}")

print("\n" + "="*90)
print("评估标准:")
print("  ✓ FWD 0.5 > FWD 0.3 > FWD 0.1 > STAND ≈ 0 > BWD")
print("  ✓ STAND 前进偏差 < 0.15m")
print("  ✓ 高度保持 > 0.15m（没有摔倒）")
print("="*90)
